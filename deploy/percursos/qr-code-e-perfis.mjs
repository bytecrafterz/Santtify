...
await pg.goto(`${SITE}/${PROJ}/perfil`,{waitUntil:'domcontentloaded'});await pg.waitForURL(/\/pessoa\//,{timeout:20000}).catch(()=>{});await pg.waitForTimeout(4500);await limpar()
p_('"Meu Perfil" abre o perfil visto de fora', /\/pessoa\//.test(pg.url()), pg.url().replace(SITE,''))
p_('e a pagina de definicoes antiga nao aparece', !/Conteúdos que você abriu|Vezes que você compartilhou/i.test(await pg.textContent('body')))
...
const id=await pg.evaluate(()=>{
  // Desde 17/09 "Meu Perfil" abre o perfil visto de fora: o @ vive em .identidade-da-pessoa.
  const a=document.querySelector('.escolher-identificador'), p=document.querySelector('.identidade-da-pessoa .identificador-perfil')
